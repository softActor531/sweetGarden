...
if (!class_exists('SatineElatedStickyNavigationWalker')) {
	class SatineElatedStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
            ...
            if($depth == 0){
                $out_div = '<div class="second '.$background_image_class.'"><div class="inner" '.($menu_image_background != '' ? satine_elated_get_inline_style($menu_image_background) : '').'>';
            }else{
                $out_div = '';
            }

            // build html
            $output .= "\n" . $indent . $out_div  .'<ul '.($menu_image_background != '' ? satine_elated_get_inline_style($menu_image_background) : '').'>' . "\n";
		}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->hide == ""){
                ...
                if($item->featured_icon !== ""){
                    $featured_icon .= '<i class="eltdf-menu-featured-icon '.$item->featured_icon .'"></i>';
                }
				...
				if($args->has_children && $depth == 0) {
					$item_output .= '<i aria-hidden="true" class="eltdf-menu-arrow arrow_triangle-down"></i>';
				}

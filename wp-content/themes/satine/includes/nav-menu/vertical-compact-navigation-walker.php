 ...
 */
if(!class_exists('SatineElatedVerticalCompactNavigationWalker')) {
	class SatineElatedVerticalCompactNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl(&$output, $depth = 0, $args = array()) {
			...
			if($depth == 0) {
				$out_div = '<div class="second '.$background_image_class.'" '.satine_elated_get_inline_style($menu_image_background).'><div class="inner">';
			} else {
				$out_div = '';
			}
			...
			if($item->hide == '') {
                ...
                if ($depth > 0) {
                    ...
                    if ($item->featured_icon !== '') {
                        $featured_icon .= '<span class="eltdf-menu-featured-icon ' . $item->featured_icon . '" aria-hidden="true"></span>';
                    }

                    $item_output .= $featured_icon;

                    if (!empty($item->description)) {
                        $item_output .= '<span class="eltdf-menu-item-description">' . $item->description . '</span>';
                    }

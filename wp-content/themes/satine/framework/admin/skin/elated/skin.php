...
class ElatedSkin extends SatineElatedSkinAbstract {
    /**
     * Skin constructor. Hooks to satine_elated_admin_scripts_init and satine_elated_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'elated';

        //hook to
        add_action('satine_elated_admin_scripts_init', array($this, 'registerStyles'));
        add_action('satine_elated_admin_scripts_init', array($this, 'registerScripts'));

        add_action('satine_elated_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('satine_elated_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('satine_elated_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('satine_elated_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
	        satine_elated_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
	        satine_elated_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = satine_elated_get_admin_tab();
        $active_page = $satine_Framework->eltdfOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see SatineElatedSkinAbstract::loadTemplatePart()
     */
	 ...
	 *
	 * @param SatineElatedAdminPage $page current page to load
	 * @see SatineElatedSkinAbstract::loadTemplatePart()
	 */

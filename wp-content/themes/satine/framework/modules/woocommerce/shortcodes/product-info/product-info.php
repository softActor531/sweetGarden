...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
        vc_map( array(
            'name' => esc_html__('Elated Product Info', 'satine'),
            'base' => $this->getBase(),
            ...
            'params' => array(
	                ...
	                array(
		                ...
		                'heading'    => esc_html__('Product Info Type', 'satine'),
		                'value'      => array(
					        esc_html__('Title', 'satine') => 'title',
					        esc_html__('Featured Image', 'satine') => 'featured_image',
					        esc_html__('Category', 'satine') => 'category',
					        esc_html__('Excerpt', 'satine') => 'excerpt',
					        esc_html__('Price', 'satine') => 'price',
					        esc_html__('Rating', 'satine') => 'rating',
					        esc_html__('Add To Cart', 'satine') => 'add_to_cart',
					        esc_html__('Tag', 'satine') => 'tag',
					        esc_html__('Author', 'satine') => 'author',
					        esc_html__('Date', 'satine') => 'date',
		                ),
		                'admin_label' => true
	                ),
	                array(
		                ...
		                'heading'    => esc_html__('Product Info Color', 'satine'),
	                ...
	                array(
		                ...
		                'heading'     => esc_html__('Category Tag', 'satine'),
		                'value'       => array_flip(satine_elated_get_title_tag(true)),
		                'description' => esc_html__('Set category tag for product category element', 'satine'),
	                ...
	                array(
		                ...
		                'heading'    => esc_html__('Add To Cart Skin', 'satine'),
		                'value'      => array(
			                esc_html__('Default', 'satine') => '',
			                esc_html__('White', 'satine')   => 'white',
		            ...
		            array(
			            ...
			            'heading'     => esc_html__('Add To Cart Size', 'satine'),
			            'value'       => array(
				            esc_html__('Default', 'satine') => '',
			            ...
			            'dependency'  => array('element' => 'product_info_type', 'value' => array('satine')),
			            'group'       => esc_html__('Product Info Style', 'satine')
		            )
				)
			)
	...
	public function getItemTitleHtml($params){
		...
		if(!empty($title)) {
			$html = '<'.esc_attr($title_tag).' itemprop="name" class="eltdf-pi-title entry-title" '.satine_elated_get_inline_style($product_info_style).'>';
    ...
    */
			...
			$html = '<div class="eltdf-pi-category">';
				foreach ($categories as $cat) {
					if(!empty($params['category_tag'])) {
						$html .= '<'.esc_attr($params['category_tag']).' '.satine_elated_get_inline_style($product_info_style).'>';
						...
						$html .= '<a itemprop="url" class="eltdf-pi-category-item" href="'.esc_url(get_term_link($cat->term_id)).'" '.satine_elated_get_inline_style($product_info_style).'>'.esc_html($cat->name).'</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml($params){
		...
		if(!empty($excerpt)) {
			$html = '<div class="eltdf-pi-excerpt"><p itemprop="description" class="eltdf-pi-excerpt-item" '.satine_elated_get_inline_style($product_info_style).'>'.esc_html($excerpt).'</p></div>';
		}
	...
	public function getItemPriceHtml($params){
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="eltdf-pi-price" '.satine_elated_get_inline_style($product_info_style).'>'.$price_html.'</div>';
		}
	...
	public function getItemRatingHtml($params){
		...
		if (get_option('woocommerce_enable_review_rating') !== 'no') {
			...
			$html = '<div class="eltdf-pi-rating" title="'.sprintf(esc_html__("Rated %s out of 5", "satine"), $average ).'" '.satine_elated_get_inline_style($product_info_style).'><span style="width:'.(($average / 5)*100).'%"></span></div>';
		}
	...
	public function getItemAddToCartHtml($params){
		...
		$html .= apply_filters( 'satine_elated_product_info_add_to_cart_link',
	...
	public function getItemTagHtml($params){
		...
		if(!empty($tags)) {
			...
			foreach ($tags as $tag) {
				$html .= '<a itemprop="url" class="eltdf-pi-tag-item" href="'.esc_url(get_term_link($tag->term_id)).'" '.satine_elated_get_inline_style($product_info_style).'>'.esc_html($tag->name).'</a>';
			}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml($params){
		...
		if(!empty($author)) {
			$html = '<div class="eltdf-pi-author" '.satine_elated_get_inline_style($product_info_style).'>'.esc_html($author).'</div>';
		}
	...
	public function getItemDateHtml($params){
		...
		if(!empty($date)) {
			$html = '<div class="eltdf-pi-date" '.satine_elated_get_inline_style($product_info_style).'>'.esc_html($date).'</div>';
		}

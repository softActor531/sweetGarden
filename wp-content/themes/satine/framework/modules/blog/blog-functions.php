 ...
 * FUNCTIONS LIST
 * 1)  @see satine_elated_include_blog_helper_functions
 * 2)  @see satine_elated_get_archive_blog_list_layout
 * 3)  @see satine_elated_get_holder_params_blog
 * 4)  @see satine_elated_get_blog
 * 5)  @see satine_elated_get_blog_type
 * 6)  @see satine_elated_get_blog_query
 * 7)  @see satine_elated_get_blog_list_holder_classes
 * 8)  @see satine_elated_get_blog_holder_data_params
 * 9)  @see satine_elated_set_ajax_url
 * 10) @see satine_elated_blog_load_more
 * 11) @see satine_elated_get_post_format_html
 * 12) @see satine_elated_single_link_pages
 * 13) @see satine_elated_get_blog_single
 * 14) @see satine_elated_get_blog_single_type
 * 15) @see satine_elated_get_single_post_format_html
 * 16) @see satine_elated_text_crop
 * 17) @see satine_elated_excerpt
 * 18) @see satine_elated_excerpt_length
 * 19) @see satine_elated_post_has_read_more
 * 20) @see satine_elated_modify_read_more_link
 * 21) @see satine_elated_get_blog_related_post_type
 * 22) @see satine_elated_get_blog_related_posts
 * 23) @see satine_elated_blog_shortcode_load_more
 * 24) @see satine_elated_get_user_custom_fields
 * 25) @see satine_elated_blog_item_has_link
 * 26) @see satine_elated_get_blog_module
 * 27) @see satine_elated_return_post_format
 * 28) @see satine_elated_return_has_media
 * 29) @see satine_elated_blog_single_title_module
 * 30) @see satine_elated_full_height_title_class

**/

if( !function_exists('satine_elated_include_blog_helper_functions') ) {
	/**
	 * Function which include blog helper function file
	 */
	function satine_elated_include_blog_helper_functions($module, $type) {
		include_once ELATED_FRAMEWORK_MODULES_ROOT_DIR.'/blog/templates/'. $module .'/' . $type . '/helper.php';
	}
}

if( !function_exists('satine_elated_get_archive_blog_list_layout') ) {
	/**
	 * Function which return archive blog list layout
	 */
	function satine_elated_get_archive_blog_list_layout() {
		$blog_layout = satine_elated_options()->getOptionValue('blog_list_type');
		
		return $blog_layout;
	}
}

if( !function_exists('satine_elated_get_holder_params_blog') ) {
	/**
	 * Function which return holder class and holder inner class for blog pages
	 *
	 * @return array
	 */
	function satine_elated_get_holder_params_blog() {
		$params = array();
         ...
         *
         */
        $params = apply_filters('satine_elated_blog_holder_params', $params);

		return $params;
	}
}

if( !function_exists('satine_elated_get_blog') ) {
	/**
	 * Function which return holder for all blog lists
	 *
	 * @return holder.php template
	 */
	function satine_elated_get_blog($type) {
		$holder_classes = '';
		$holder_classes = apply_filters('satine_elated_blog_holder_classes', $holder_classes);
		...
		satine_elated_get_module_template_part('templates/lists/holder', 'blog', '', $params);
	}
}

if( !function_exists('satine_elated_get_blog_type') ) {
	/**
	 * Function which create query for blog lists
	 * @param $type type of list that is loaded
	 * @return blog list template
	 */
	function satine_elated_get_blog_type($type) {
        $blog_query = satine_elated_get_blog_query();
		$paged = isset($blog_query->query['paged']) ? $blog_query->query['paged'] : 1;
		$max_num_pages = $blog_query->max_num_pages;
		
        $blog_classes = satine_elated_get_blog_list_holder_classes($type);
        $blog_data_params = satine_elated_get_blog_holder_data_params($type);
		...
		satine_elated_get_module_template_part('templates/lists/' .  $type . '/list', 'blog', '', $params);
	}
}

if(!function_exists('satine_elated_get_blog_query')){
	/**
	* Function which create query for blog lists
	*
	* @return wp query object
	*/
	function satine_elated_get_blog_query(){
...
if(!function_exists('satine_elated_get_max_number_of_pages')) {
    /**
     ...
     * @version 0.1
     */
    function satine_elated_get_max_number_of_pages() {
        global $wp_query;
...
if(!function_exists('satine_elated_get_blog_list_holder_classes')){
	/**
	 * Function set blog list classes
	 * @param $type - type of blog list that is passing
	 * @return array - array of classes
	 */
	function satine_elated_get_blog_list_holder_classes($type){
		$blog_classes   = array();
        ...
        $masonry_type = satine_elated_get_meta_field_intersect('blog_list_featured_image_proportion');
        if(!empty($masonry_type)) {
            $blog_classes[] = 'eltdf-masonry-images-' . $masonry_type;
        }
		
		$blog_classes = apply_filters('satine_elated_blog_list_classes', $blog_classes);
		
		return implode(' ', $blog_classes);
	}
}

if(!function_exists('satine_elated_get_blog_holder_data_params')){
	/**
	 ...
	 */
	function satine_elated_get_blog_holder_data_params($type){
		$current_query = satine_elated_get_blog_query();
		...
		$category = get_post_meta(satine_elated_get_page_id(), 'eltdf_blog_category_meta', true);
...
if(!function_exists('satine_elated_set_ajax_url')){
	/**
	 * load themes ajax functionality
	 */
	function satine_elated_set_ajax_url() {
		echo '<script type="application/javascript">var ElatedAjaxUrl = "'.admin_url('admin-ajax.php').'"</script>';
	}
	
	add_action('wp_enqueue_scripts', 'satine_elated_set_ajax_url');
}

if(!function_exists('satine_elated_blog_load_more')){
	
	function satine_elated_blog_load_more(){
		$return_obj = array();
		...
		if($query_results->have_posts()):
			while ( $query_results->have_posts() ) : $query_results->the_post();
				$html .= satine_elated_get_post_format_html($blog_type, 'yes');
			endwhile;
		else:
			$html .= satine_elated_get_module_template_part('templates/parts/no-posts', 'blog');
		endif;
    ...
    add_action('wp_ajax_nopriv_satine_elated_blog_load_more', 'satine_elated_blog_load_more');
    add_action( 'wp_ajax_satine_elated_blog_load_more', 'satine_elated_blog_load_more' );
}

if( !function_exists('satine_elated_get_post_format_html') ) {
	/**
	 ...
	 */
	function satine_elated_get_post_format_html($type = "", $ajax = '') {

        $post_format = satine_elated_return_post_format();
        ...
        $post_classes = array();
        $post_classes[] = satine_elated_return_has_media() ? 'eltdf-post-has-media' : 'eltdf-post-no-media';
         ...
         *
         */
        $params['part_params']        = apply_filters('satine_elated_blog_part_params', array());

		if($ajax == ''){
			satine_elated_get_module_template_part('templates/lists/' . $type . '/post', 'blog', $post_format, $params);
		}
		if($ajax == 'yes'){
			return satine_elated_get_blog_module_template_part('templates/lists/' . $type . '/post', $post_format, $params);
		}
	}
}

if( !function_exists('satine_elated_single_link_pages') ) {
    /**
     * Function which return parts on single.php which are just below content
     *
     * @return single.php html
     */
    function satine_elated_single_link_pages() {
    ...
    add_action('satine_elated_single_link_pages', 'satine_elated_single_link_pages');
}

if(!function_exists('satine_elated_get_blog_single')) {
	/**
	 ...
	 */
	function satine_elated_get_blog_single($type) {
		$sidebar_layout = satine_elated_sidebar_layout();
		
		$holder_classes = $sidebar_layout !== 'no-sidebar' ? 'eltdf-content-has-sidebar' : '';
		$holder_classes = apply_filters('satine_elated_blog_single_holder_classes', $holder_classes);
		...
		satine_elated_get_module_template_part('templates/singles/holder', 'blog', '', $params);
	}
}

if( !function_exists('satine_elated_get_blog_single_type') ) {
    /**
     * Function which create query for blog lists
     * @param $type type of list that is loaded
     * @return blog list template
     */
    function satine_elated_get_blog_single_type($type) {
         ...
         *
         */
        $params['single_info_params'] = apply_filters('satine_elated_blog_single_info_params', array());

        satine_elated_get_module_template_part('templates/singles/'.$type.'/single', 'blog', '', $params);
    }
}

if( !function_exists('satine_elated_get_single_post_format_html') ) {
	/**
	 ...
	 * @return single.php html
	 */
	function satine_elated_get_single_post_format_html($type) {
        $post_format = satine_elated_return_post_format();
		$params = array();
        /*
         * Available parameters for template parts
        ...
        satine_elated_get_module_template_part('templates/singles/'.$type.'/'.$post_format, 'blog', '', $params);
	}
}

if(!function_exists('satine_elated_text_crop')) {
    /**
     * Function that cuts plain text to the number of words supplied
     *
     */
    function satine_elated_text_crop($text, $word_count = 45, $suffix = true) {
...
if(!function_exists('satine_elated_excerpt')) {
    /**
     ...
     *
     */
    function satine_elated_excerpt($length) {
        global $post;
        ...
        elseif(satine_elated_post_has_read_more()) {
            global $more;
        ...
        $number_of_chars = satine_elated_get_meta_field_intersect('number_of_chars', satine_elated_get_page_id());
...
if (!function_exists('satine_elated_excerpt_length')) {
	/**
	 * Function that changes excerpt length based on theme options
	 * @return int changed value
	 */
	function satine_elated_excerpt_length() {
		$numb_of_chars = satine_elated_options()->getOptionValue('number_of_chars');
		
		return $numb_of_chars !== '' ? $numb_of_chars : 45;
	}

	add_filter( 'excerpt_length', 'satine_elated_excerpt_length', 999 );
}

if(!function_exists('satine_elated_post_has_read_more')) {
	/**
	 * Function that checks if current post has read more tag set
	 * @return int position of read more tag text. It will return false if read more tag isn't set
	 */
	function satine_elated_post_has_read_more() {
		global $post;
...
if (!function_exists('satine_elated_modify_read_more_link')) {
	/**
	 * Function that modifies read more link output.
	 * Hooks to the_content_more_link
	 * @return string modified output
	 */
	function satine_elated_modify_read_more_link() {
		$link = '<div class="eltdf-more-link-container">';
        if(satine_elated_core_plugin_installed()) {
            $link .= satine_elated_get_button_html(array(
                'link' => get_permalink() . '#more-' . get_the_ID(),
                'text' => esc_html__('Continue reading', 'satine')
            ));
        } else {
            ...
            $link .= esc_html__('Continue reading', 'satine');
            $link .= '</span></a>';
        }
	...
	add_filter( 'the_content_more_link', 'satine_elated_modify_read_more_link');
}

if ( ! function_exists('satine_elated_get_blog_related_post_type')) {
    /**
     ...
     * @return WP_Query
     */
    function satine_elated_get_blog_related_post_type($post_id, $options = array()) {
        ...
        if ($tag_ids) {
            $related_by_tag = satine_elated_get_blog_related_posts($post_id, $tag_ids, 'tag', $options);
        ...
        if ($categories && !$hasRelatedByTag) {
            $related_by_category = satine_elated_get_blog_related_posts($post_id, $category_ids, 'category', $options);
...
if ( ! function_exists('satine_elated_get_blog_related_posts') ) {
    /**
     ...
     * @return WP_Query
     */
    function satine_elated_get_blog_related_posts($post_id, $term_ids, $slug, $options = array()) {
        //Query options
        $posts_per_page = -1;
...
if(!function_exists('satine_elated_blog_shortcode_load_more')){

    function satine_elated_blog_shortcode_load_more(){
        $params = array();
        ...
        if($query_results->have_posts()):
            while ( $query_results->have_posts() ) : $query_results->the_post();
                satine_elated_get_module_template_part('shortcodes/blog-list/layout-collections/'.$params['type'], 'blog', '', $params);
            endwhile;
        else:
            satine_elated_get_module_template_part('templates/parts/no-posts', 'blog', '', $params);
        endif;
    ...
    add_action('wp_ajax_nopriv_satine_elated_blog_shortcode_load_more', 'satine_elated_blog_shortcode_load_more');
    add_action( 'wp_ajax_satine_elated_blog_shortcode_load_more', 'satine_elated_blog_shortcode_load_more' );
}

if(!function_exists('satine_elated_get_user_custom_fields')) {
	/**
	 * Function returns links and icons for author social networks
	 *
	 * return array
	 */
	function satine_elated_get_user_custom_fields() {
...
if(!function_exists('satine_elated_blog_item_has_link')) {
	/**
	 * Function returns true/false depends is single post or in loop
	 *
	 * return boolean
	 */
	function satine_elated_blog_item_has_link() {
		$is_link = (is_single() && (get_the_ID() === satine_elated_get_page_id())) ? false : true;

		return $is_link;
	}
}

if(!function_exists('satine_elated_get_blog_module')) {
    /**
     * Function returns single/list depending is single post or in loop
     *
     * return string
     */
    function satine_elated_get_blog_module() {
        $module = (is_single() && (get_the_ID() === satine_elated_get_page_id())) ? 'single' : 'list';

        return $module;
    }
}

if( !function_exists('satine_elated_return_post_format') ) {
    /**
     * Function return all parts on single.php page
     *
     * @return string with post format
     */
    function satine_elated_return_post_format() {
...
if( !function_exists('satine_elated_return_has_media') ) {
    /**
     * Function return all parts on single.php page
     *
     * @return string with post format
     */
    function satine_elated_return_has_media() {
        $post_format = get_post_format();
...
if( !function_exists('satine_elated_blog_single_title_module') ) {
	/**
	 * Function that replaces title image with single post featured image on single post page
	 *
	 * @see satine_elated_get_title() from title-functions.php file for list of available params
	 *
	 * @param array - array of default title values
	 *
	 * @return array of overridden values
	 */
	function satine_elated_blog_single_title_module($params) {
		...
		$show_title_area_blog_single = satine_elated_get_meta_field_intersect('show_title_area_blog', get_the_ID());
		...
		$background_image = '';
		$title_area_background_image_meta = get_post_meta(get_the_ID(), 'eltdf_title_area_background_image_meta', true);
		...
		if (!empty($background_image)) {
			...
			$is_img_responsive = satine_elated_get_meta_field_intersect('title_area_background_image_responsive', get_the_ID());
...
if(!function_exists('satine_elated_full_height_title_class')) {
    /**
     * Function that adds class on title holder if full height title option is enabled for single post page
     *
     * @see satine_elated_title_classes() in filter-functions.php
     *
     * @param $classes array of classes
     *
     * @return array changed array of classes
     */
    function satine_elated_full_height_title_class($classes) {

        $full_height_title_area_meta = satine_elated_get_meta_field_intersect('full_height_title_area_blog', get_the_ID());
        $classes[] = $full_height_title_area_meta == 'yes' ? 'eltdf-title-full-height' : "";

        return $classes;
    }

    add_filter('satine_elated_title_classes', 'satine_elated_full_height_title_class');
}

if(!function_exists('satine_elated_post_has_thumbnail')) {
	/**
	 * Function that adds class on title holder if full height title option is enabled for single post page
	 *
	 * @see satine_elated_title_background_image_classes() in filter-functions.php
	 *
	 * @param $title_img string with title image url
	 *
	 * @return string with image
	 */
	function satine_elated_post_has_thumbnail($title_img) {

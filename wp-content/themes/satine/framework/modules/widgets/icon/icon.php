...
class SatineElatedIconWidget extends SatineElatedWidget {
    public function __construct() {
        parent::__construct(
            'eltdf_icon_widget',
            esc_html__('Elated Icon Widget', 'satine'),
            array( 'description' => esc_html__( 'Add icons to widget areas', 'satine'))
        );
	...
	protected function setParams() {
		$this->params = array_merge(
			satine_elated_icon_collections()->getIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'satine' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
				...
				array(
					...
					'options' => array(
						'_self'  => esc_html__( 'Same Window', 'satine' ),
    ...
    public function widget($args, $instance) {
	    ...
	    $data   = array();
	    $data[] = ! empty( $instance['icon_hover_color'] ) ? satine_elated_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
	    $data   = implode( ' ', $data );
	    ...
	    if ( ! empty( $instance['icon_size'] ) ) {
		    $holder_styles[] = 'font-size: ' . satine_elated_filter_px( $instance['icon_size'] ) . 'px';
	    }
	    ...
	    if ( ! empty( $instance['icon_text'] ) ) {
		    $icon_text_html = '<span class="eltdf-icon-text">' . esc_html( $instance['icon_text'] ) . '</span>';
	    }
	    ?>

        <a <?php satine_elated_class_attribute($holder_classes); ?> <?php echo wp_kses_post($data); ?> href="<?php echo esc_url($link); ?>" target="<?php echo esc_attr($target); ?>" <?php echo satine_elated_get_inline_style($holder_styles); ?>>

...
<form class="woocommerce-shipping-calculator" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<?php printf( '<a href="#" class="shipping-calculator-button">%s</a>', esc_html( ! empty( $button_text ) ? $button_text : __( ' 	Delivery Cost', 'woocommerce' ) ) ); ?>
 ...
 <input id="edValue" type="text" onInput="edValueKeyPress()" placeholder="<?php esc_attr_e( 'POSTAL CODE', 'woocommerce' ); ?>" value="<?php echo esc_attr( WC()->customer->get_shipping_city() ); ?>">
				<input type="hidden" class="input-text" value="<?php echo esc_attr( WC()->customer->get_shipping_city() ); ?>" placeholder="<?php esc_attr_e( 'POSTALCODE', 'woocommerce' ); ?>" name="calc_shipping_city" id="calc_shipping_city" />
			</p>
		<?php endif; ?>
...
<script>
  ...
  function edValueKeyPress() {
    ...
    var s = edValue.value;
    var variable2 = s.substring(0, 3);
   document.getElementById("calc_shipping_city").value = s;
    document.getElementById("calc_shipping_city").value = variable2;
}
</script>

...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            ...
            'params'                    => array_merge(
                \SatineElatedIconCollections::get_instance()->getVCParamsArray(),
                array(
                    array(
                        'type'       => 'dropdown',
                        'param_name' => 'size',
    ...
    private function generateIconStyles($params) {
        ...
        if(($params['type'] !== 'eltdf-normal' && !empty($params['shape_size'])) || ($params['type'] == 'eltdf-normal')) {
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.satine_elated_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'eltdf-normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.satine_elated_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.satine_elated_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.satine_elated_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.satine_elated_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'eltdf-square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.satine_elated_filter_px($params['border_radius']).'px';
                }
            }
        }
